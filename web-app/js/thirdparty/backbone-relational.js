    ...
    Backbone.RelationalModel = Backbone.Model.extend({
        ...
        toJSONFull: function() {
            ...
            _.each( this._relations, function( rel ) {
                ...
                if (value && _.isFunction( value.toJSON ) ) {//by default convert full
                    json[ rel.keySource ] = value.toJSON();
                }
                else if ( _.isString( rel.options.includeInJSON ) ) {
                    if ( value instanceof Backbone.Collection ) {
        ...
        toJSONFullSecondOrder: function() {
            // If this Model has already been fully serialized in this branch once, return to avoid loops
            if ( this.isLocked() ) {
                return this.id;
            }
            ...
            _.each( this._relations, function( rel ) {
                ...
                if (value && _.isFunction( value.toJSONFull ) ) {//by default convert full
                    json[ rel.keySource ] = value.toJSONFull();
                }
                else if ( _.isString( rel.options.includeInJSON ) ) {
                    if ( value instanceof Backbone.Collection ) {
                        json[ rel.keySource ] = value.pluck( rel.options.includeInJSON );
